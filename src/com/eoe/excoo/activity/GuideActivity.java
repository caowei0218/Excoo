...
public class GuideActivity extends BaseActivity {
	...
	private void iniViewPager() {
		...
		for (int i = 0; i < mViews.size(); i++) {
			...
			if (i == 0) {
				indicator.setImageResource(R.drawable.point);
			} else {
	...
	private class MyPagerChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			for (int i = 0; i < mViews.size(); i++) {
				mIndicatorList.get(i).setImageResource(R.drawable.point01);
			}
			mIndicatorList.get(position).setImageResource(R.drawable.point);
		}
	}
}
